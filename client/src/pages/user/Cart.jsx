...
// Unique key per cart row — productId alone is not enough when variants exist
const itemKey = (item) =>
  `${item.productId}__${item.variantIndex ?? "null"}`;
...
export default function CartPage() {
  ...
  const handleQty = async (item, delta) => {
    ...
    try {
      ...
      setItems(prev =>
        prev.map(i => itemKey(i) === key ? { ...i, quantity: newQty } : i)
      );
    } catch {
  ...
  const handleCheckout = () => {
    ...
    // FIX: Warn early if any item is missing sellerAddress — prevents the
    // cryptic "missing sellerAddress" error from appearing mid-checkout.
    const missing = selectedItems.filter(i => !i.sellerAddress);
    if (missing.length > 0) {
      setError(
        `Some items are missing seller info: ${missing.map(i => i.name).join(", ")}. ` +
  ...
  // FIX: Count unique sellers by sellerAddress (wallet address), not sellerId/productId.
  // Items without sellerAddress (old cart rows) count as 1 unknown seller group.
  const uniqueSellers = selectedItems.some(i => i.sellerAddress)
    ? new Set(selectedItems.map(i => i.sellerAddress?.toLowerCase()).filter(Boolean)).size
            ...
            <div className="lg:col-span-2 space-y-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 px-5 py-3 flex items-center justify-between shadow-sm">
                <button onClick={toggleAll}
                  className="flex items-center gap-2.5 text-sm font-semibold text-gray-700 hover:text-green-600 transition-colors">
                  {allSelected
                  ...
                  <div key={key}
                    ...
                    <div className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* FIX: Warn inline if this item is missing sellerAddress (old cart row) */}
                        {!item.sellerAddress && (
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>Logistics ({uniqueSellers} seller{uniqueSellers !== 1 ? "s" : ""} × 50)</span>
